...
bool CFileDirHandle::mkPathIfNotExist(const QString &strPath)
{
    QDir dir;
    bool ret = dir.exists(strPath);
    if (!ret) {
        ret = dir.mkpath(strPath);
    }
    return ret;
...
QString CFileDirHandle::getFileCompleteSuffix(const QString &strFilePath)
{
...
QString CFileDirHandle::generateFileNameBySuffix(const QString &oldFileName, const QString &newFileSuffix)
{
    ...
    if(newFileName.endsWith(".") && newFileSuffix.startsWith("."))
    {
        newFileName = newFileName.left(newFileName.length() - 1);
    }
...
* 作者: Joker
*/
QString CFileDirHandle::getAppBasicDataDir()
{
#ifdef Q_OS_MAC
